...
public class TchatService {
	...
	public Response getMessages(MessageBean message) {
		...
		ArrayList<MessageBean> maListe = MessageBDD.getMessages();
		return Response.status(200).entity(gson.toJson(maListe)).build();
	...
	public void sendMessages(String recu) {
		...
		if (recu == null || recu.length() == 0) {
			...
			MessageBean message;
			message = gson.fromJson(recu, MessageBean.class);

			MessageBDD.addMessages(message);
		}
	}/*http://localhost:8080/Nolan/rest/Webservice/sendMessages*/
}